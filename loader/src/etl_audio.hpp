    ...
    */
    ...
    public:
        ...
        * "samples" (e.g. .01 seconds). */
        ...
        /** Window type for spectrogram generation (What are the options? Only used for specgram?)*/
        std::string window_type      {"hann"};

        std::string noise_index_file {};

        /** Sample rate of input audio in hertz (what happens if it's not this?)*/
        uint32_t    sample_freq_hz   {16000};

        /** Not sure what this does */
        std::uniform_real_distribution<float>    time_scale_fraction   {1.0f, 1.0f};

        /** How much noise to add (what are the units? why the range to 0.5?) */
        ...
        /** Whether or not to add noise, yes? */
